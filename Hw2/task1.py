...
def main(args):
    ...
    if case_num == 1:
        case_rdd = input_rdd.map(lambda x: (str(x[0]), str(x[1]))).aggregateByKey([], lambda a,b: a + [b], lambda a,b: a + b).map(lambda x: (x[0],sorted([*set(x[1])])))
    elif case_num == 2:
        case_rdd = input_rdd.map(lambda x: (str(x[1]), str(x[0]))).aggregateByKey([], lambda a,b: a + [b], lambda a,b: a + b).map(lambda x: (x[0],sorted([*set(x[1])])))
    else:
        print('Invalid case number')
        sc.stop()
        return
    print(case_rdd.collect())
    num_partitions = case_rdd.getNumPartitions()
    canidates = case_rdd.mapPartitions(lambda x: pass1_map(x, support_thr, num_partitions)).reduceByKey(lambda a,b: a).map(lambda x: x[0]).sortBy(lambda x: (len(x),x)).collect()
...
def subsets(basket):
    return chain.from_iterable(combinations(basket, r) for r in range(2, 7))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='A2T1')
    parser.add_argument('--input_file', type=str, default='./data/hw2/small1.csv', help='the input file')
    parser.add_argument('--output_file', type=str, default='./data/hw2/a2t1.json',
